...
class ModelManager:
    ...
    def select_model(self, model_type, previous_models=None):
        ...
        available_models = [
            ...
            if model not in previous_models and 
            (model not in self.request_timeouts or 
             current_time - self.request_timeouts.get(model, 0) > 60)
        ]
    ...
    def call_model(self, model_type, prompt, previous_models=None, system_prompt=None):
        ...
        try:
            ...
            if len(previous_models) < len(PLANNING_MODELS if model_type == 'planning' 
                                      else CODING_MODELS if model_type == 'coding'
                                      else CHECKING_MODELS):
                self.logger.info(f"Falling back to next available {model_type} model")
                return self.call_model(model_type, prompt, previous_models, system_prompt)
            else:

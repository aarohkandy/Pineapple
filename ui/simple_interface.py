...
class SimpleInterface:
    ...
    def _display_result(self, result):
        ...
        if result.get("plan"):
            ...
            if "raw_plan" in result["plan"]:
                ...
                print(plan_summary[:500] + "..." if len(plan_summary) > 500 else plan_summary)
                ...
                if result["plan"].get("requirements"):
                    ...
                    if len(result["plan"]["requirements"]) > 5:
                        print(f"- ...and {len(result['plan']['requirements']) - 5} more")
        ...
        if result.get("verification"):
            ...
            if result["verification"].get("issues"):
                ...
                if len(result["verification"]["issues"]) > 3:
                    print(f"- ...and {len(result['verification']['issues']) - 3} more issues")
            else:
                print("\nNo issues found.")
    ...
    def _save_files(self, result):
        ...
        """
        if not result or not result.get("code") or not result["code"].get("file_structure"):

...
class CheckingAgent:
    ...
    def verify_code(self, code: str | dict, requirements: list | None = None) -> dict:
        ...
        if isinstance(code, dict) and any(
            filename.endswith(".py") for filename in code.keys()
        ):
        ...
        self.logger.info(
            f"Code verification complete. Success: {verification_result['success']}"
        )
        return verification_result
    ...
    def _static_code_analysis(
        ...
        checking_prompt = CHECKING_PROMPT_TEMPLATE.format(
            code=formatted_code, requirements=formatted_requirements
        )
        ...
        if not response["success"]:
            self.logger.error(
                "Code checking failed: " + response.get("error", "Unknown error")
            )
            return {
                "success": False,
    ...
    def _parse_checking_response(self, content: str) -> dict:
        ...
        verdict_match = re.search(
            r"(?:verdict|conclusion|summary)[\s:]*([^\n]+)", content, re.IGNORECASE
        )
        verdict = (
            verdict_match.group(1).strip()
            if verdict_match
            else "No clear verdict provided."
        )
        ...
        issues_section = re.search(
            r"(?:issues|problems|bugs|errors):([\s\S]*?)(?=##|\Z)",
            content,
            re.IGNORECASE,
        )
        issues = []
        if issues_section:
            ...
            issues = [
                ...
                if line.strip()
                and any(line.strip().startswith(bullet) for bullet in ["-", "*", "•"])
            ]

        # Extract improvements/suggestions
        improvements_section = re.search(
            r"(?:improvements|suggestions|recommendations):([\s\S]*?)(?=##|\Z)",
            content,
            re.IGNORECASE,
        )
        improvements = []
        ...
        return {
            "success": success
            and not any(
                re.search(r"critical|severe|major", issue, re.IGNORECASE)
                for issue in issues
            ),
            "verdict": verdict,
            "issues": issues,
    ...
    def _execute_code(self, code: str) -> dict:
        ...
        if execution_result["success"]:
            ...
            self.logger.warning(
                f"Code execution failed: {execution_result.get('error', 'Unknown error')}"
            )

...
class PlanningAgent:
    ...
    def create_plan(self, user_prompt):
        ...
        if not response["success"]:
            self.logger.error("Planning failed: " + response.get("error", "Unknown error"))
            return {
                "success": False,
                "error": response.get("error", "Planning failed")
            }
        ...
        try:
            ...
            if not plan_data:
                self.logger.warning("Could not parse planning response as JSON, using raw text")
                return {
                    "success": True,
                    "raw_plan": content,
                    "plan": content,
    ...
    def _extract_planning_data(self, content):
        ...
        try:
            ...
            requirements_section = re.search(r'## Requirements([\s\S]*?)(?=##|$)', content)
            tasks_section = re.search(r'## Tasks([\s\S]*?)(?=##|$)', content)
            
            result = {
                "plan": plan_section.group(1).strip() if plan_section else "",
                "research": research_section.group(1).strip() if research_section else "",
                "requirements": self._extract_list(requirements_section.group(1) if requirements_section else ""),
                "task_breakdown": self._extract_list(tasks_section.group(1) if tasks_section else "")
            }

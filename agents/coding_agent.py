...
class CodingAgent:
    ...
    def generate_code(self, user_prompt: str, plan: str, research: str) -> dict:
        ...
        coding_prompt = CODING_PROMPT_TEMPLATE.format(
            user_prompt=user_prompt, plan=plan, research=research
        )
        ...
        if not response["success"]:
            self.logger.error(
                "Code generation failed: " + response.get("error", "Unknown error")
            )
            return {
                "success": False,
        ...
        if not code_blocks:
            ...
            return {
                ...
                "code": response[
                    "content"
                ],  # Return the raw content if no code blocks found
                "explanation": "",
                "file_structure": {},
            }
        ...
        self.logger.info(
            f"Successfully generated code with {len(file_structure)} files"
        )
        return {
            "success": True,
